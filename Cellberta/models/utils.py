...
def load_trained_model(model: BaseModel, model_configs: ModelConfigs, is_training: bool) -> BaseModel:
    ...
    checkpoint = torch.load(
        checkpoint_path, map_location=model.image_model.device
    )
    ...
    for name, param in checkpoint.items():
        if (
            "image_model.base_model.model.encoder.layer" in name
    ...
    if model_configs.image_fine_tuning_type in [
        "lora",
        "lokr",
        "loha",
        "ia3",
    ]:
        print("Merging image model with its adapter")
        model.image_model.merge_and_unload()
...
def load_pretrained_activity_bounds(checkpoint_path: str) -> Tuple[float, float]:
    """
    Load pre-defined activity bounds based on the specific checkpoint.

    Args:
        checkpoint_path (str): Path to the model checkpoint.

    Returns:
        Tuple[float, float]: The lower and upper bounds of activity values for the given dataset.

    Raises:
        ValueError: If the checkpoint path does not match any known activity scale.
    """
    if "cpd" in checkpoint_path:
        # Compound activity scale
        activity_lower_bound = 0.0
        activity_upper_bound = 1.0
    elif "cell" in checkpoint_path:
        activity_lower_bound = -2.0
        activity_upper_bound = 2.0
    elif "combined" in checkpoint_path:
        activity_lower_bound = -1.0
        activity_upper_bound = 1.0
    else:
        # Raise an error if an unknown activity scale is encountered
        raise ValueError(
            f"Unknown activity scale, for {checkpoint_path}"
        )
    
    return activity_lower_bound, activity_upper_bound